		...
		calculate = function(self, card, context)
			if context.end_of_round and not context.blueprint and not context.repetition and not context.individual then
				if G.GAME.current_round.hands_left + G.GAME.current_round.discards_left > 0 then
					card.ability.extra.mulchs = card.ability.extra.mulchs
			...
			elseif
				...
				and card.ability.extra.mulchs > 1
			then
				return {
				...
				vars = {
					...
					to_big(1)
						+ to_big(card.ability.extra.Xmult)
							* math.floor(
								(G.GAME.dollars + (G.GAME.dollar_buffer or to_big(0)))
	...
	SMODS.Joker({ --Hold Your Breath
		...
		calculate = function(self, card, context)
			...
			elseif
				context.after
				and not context.blueprint
				and card.ability.extra.chips > card.ability.extra.chip_limit
			then
				G.E_MANAGER:add_event(Event({
					func = function()
						play_sound("tarot1")
	...
	SMODS.Joker({ --Prideful Joker
		...
		calculate = function(self, card, context)
			if
				context.cardarea == G.play
				and context.individual
				and SMODS.get_enhancements(context.other_card)["m_wild"] == true
			then
				return {
			...
			elseif context.before and not context.blueprint then
				...
				if not card.ability.extra.fished then
					for i = 1, #context.scoring_hand do
						if
							context.scoring_hand[i].base.value == G.GAME.current_round.fish_rank.rank
						...
						G.E_MANAGER:add_event(Event({
							func = function()
								G.E_MANAGER:add_event(Event({
									func = function()
										local card =
											create_card("Planet", G.consumeables, nil, nil, nil, nil, nil, "yang")
										card:add_to_deck()
										G.consumeables:emplace(card)
										G.GAME.consumeable_buffer = 0
										return true
			...
			then
				if pseudorandom("apple") < G.GAME.probabilities.normal / card.ability.extra.odds then
					...
					local joker_to_destroy = #destructable_jokers > 0
							and pseudorandom_element(destructable_jokers, pseudoseed("apple"))
						or nil
...
SMODS.Back({ --Bazaar Deck
	...
	loc_vars = function(self, info_queue, center)
		return {
			vars = {
				localize({ type = "name_text", key = "v_clearance_sale", set = "Voucher" }),
